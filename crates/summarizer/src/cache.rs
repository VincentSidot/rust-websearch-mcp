//! Summarizer response cache implementation using sled

use crate::config::CacheConfig;
use kernel::SummarizeResponse;
use log::{debug, info, warn};
use serde::{Deserialize, Serialize};
use sled::Db;
use std::path::Path;
use std::sync::Arc;

/// Cache key format: sum:{doc_id}:{hash(payload)}
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct CacheKey {
    pub doc_id: String,
    pub selected_segment_ids: Vec<String>,
    pub style: String,
    pub map_reduce_enabled: bool,
    pub map_reduce_max_context_tokens: usize,
    pub map_reduce_map_group_tokens: usize,
    pub map_reduce_reduce_target_words: usize,
    pub model: String,
    pub base_url: String,
    pub prompt_version: String,
}

impl CacheKey {
    /// Create a new cache key
    pub fn new(
        doc_id: String,
        selected_segment_ids: Vec<String>,
        style: String,
        map_reduce_enabled: bool,
        map_reduce_max_context_tokens: usize,
        map_reduce_map_group_tokens: usize,
        map_reduce_reduce_target_words: usize,
        model: String,
        base_url: String,
        prompt_version: String,
    ) -> Self {
        // Ensure selected_segment_ids are sorted for deterministic hashing
        let mut sorted_segment_ids = selected_segment_ids;
        sorted_segment_ids.sort();
        
        Self {
            doc_id,
            selected_segment_ids: sorted_segment_ids,
            style,
            map_reduce_enabled,
            map_reduce_max_context_tokens,
            map_reduce_map_group_tokens,
            map_reduce_reduce_target_words,
            model,
            base_url,
            prompt_version,
        }
    }

    /// Convert to string key for sled
    pub fn to_string_key(&self) -> Result<String, Box<dyn std::error::Error>> {
        // Serialize the key components
        let serialized = bincode::serialize(self)?;
        
        // Hash the serialized data
        let hash = blake3::hash(&serialized);
        let hash_hex = hash.to_hex().to_string();
        
        Ok(format!("sum:{}:{}", self.doc_id, hash_hex))
    }

    /// Parse from string key (extract doc_id part)
    pub fn doc_id_from_string_key(key: &str) -> Option<String> {
        if let Some(stripped) = key.strip_prefix("sum:") {
            if let Some(pos) = stripped.find(':') {
                return Some(stripped[..pos].to_string());
            }
        }
        None
    }
}

/// Cache statistics
#[derive(Debug, Clone)]
pub struct CacheStats {
    pub entry_count: u64,
    pub total_size_bytes: u64,
}

/// Summarizer response cache using sled
pub struct SummarizerCache {
    /// Sled database instance
    db: Arc<Db>,
    
    /// Cache configuration
    config: CacheConfig,
    
    /// Cache hit counter
    hit_count: std::sync::atomic::AtomicU64,
    
    /// Cache miss counter
    miss_count: std::sync::atomic::AtomicU64,
}

impl SummarizerCache {
    /// Flush the cache database to ensure all writes are persisted
    pub fn flush(&self) -> Result<(), Box<dyn std::error::Error>> {
        self.db.flush()?;
        Ok(())
    }
    
    /// Close the cache database
    pub fn close(self) -> Result<(), Box<dyn std::error::Error>> {
        // Flush before closing
        self.db.flush()?;
        // Drop the database instance
        drop(self);
        Ok(())
    }
}

impl Drop for SummarizerCache {
    fn drop(&mut self) {
        // Flush the database to ensure all writes are persisted
        if let Err(e) = self.db.flush() {
            log::warn!("Failed to flush cache database: {}", e);
        }
    }
}

impl SummarizerCache {
    /// Create a new summarizer cache
    pub fn new(config: CacheConfig) -> Result<Self, Box<dyn std::error::Error>> {
        if !config.enabled {
            info!("Summarizer cache is disabled");
            return Ok(Self {
                db: Arc::new(sled::Config::new().temporary(true).open()?),
                config,
                hit_count: std::sync::atomic::AtomicU64::new(0),
                miss_count: std::sync::atomic::AtomicU64::new(0),
            });
        }

        // Ensure cache directory exists
        let cache_path = Path::new(&config.path);
        if let Some(parent) = cache_path.parent() {
            std::fs::create_dir_all(parent)?;
        }

        info!("Initializing summarizer cache at: {:?}", cache_path);
        let db = sled::Config::new().path(cache_path).open()?;

        Ok(Self {
            db: Arc::new(db),
            config,
            hit_count: std::sync::atomic::AtomicU64::new(0),
            miss_count: std::sync::atomic::AtomicU64::new(0),
        })
    }

    /// Get a summary response from the cache
    pub fn get_summary(
        &self,
        cache_key: &CacheKey,
    ) -> Result<Option<SummarizeResponse>, Box<dyn std::error::Error>> {
        if !self.config.enabled {
            return Ok(None);
        }

        let string_key = cache_key.to_string_key()?;
        debug!("Checking cache for key: {}", &string_key[..std::cmp::min(string_key.len(), 64)]);

        match self.db.get(&string_key)? {
            Some(value) => {
                // Try to deserialize the value
                match bincode::deserialize::<SummarizeResponse>(&value) {
                    Ok(response) => {
                        self.hit_count
                            .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                        debug!("Cache hit for key: {}", &string_key[..std::cmp::min(string_key.len(), 64)]);
                        Ok(Some(response))
                    }
                    Err(e) => {
                        // Corrupted entry, treat as miss and warn
                        warn!("Corrupted cache entry for key: {} - {:?}", &string_key[..std::cmp::min(string_key.len(), 64)], e);
                        self.db.remove(&string_key)?;
                        self.miss_count
                            .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                        Ok(None)
                    }
                }
            }
            None => {
                self.miss_count
                    .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                debug!("Cache miss for key: {}", &string_key[..std::cmp::min(string_key.len(), 64)]);
                Ok(None)
            }
        }
    }

    /// Put a summary response into the cache
    pub fn put_summary(
        &self,
        cache_key: &CacheKey,
        response: &SummarizeResponse,
    ) -> Result<(), Box<dyn std::error::Error>> {
        if !self.config.enabled {
            return Ok(());
        }

        let string_key = cache_key.to_string_key()?;
        debug!("Caching summary for key: {}", &string_key[..std::cmp::min(string_key.len(), 64)]);

        // Serialize to bytes
        let serialized = bincode::serialize(response)?;
        
        self.db.insert(&string_key, serialized)?;
        debug!("Cached summary for key: {}", &string_key[..std::cmp::min(string_key.len(), 64)]);
        Ok(())
    }

    /// Get cache statistics
    pub fn get_stats(&self) -> Result<CacheStats, Box<dyn std::error::Error>> {
        let entry_count = self.db.len() as u64;
        
        // Estimate total size by sampling a few entries
        let mut total_size_bytes = 0u64;
        let mut sampled_entries = 0u64;
        
        for result in self.db.iter().take(100) {
            let (_key, value) = result?;
            total_size_bytes += value.len() as u64;
            sampled_entries += 1;
        }
        
        // Extrapolate total size if we sampled entries
        if sampled_entries > 0 && entry_count > 0 {
            let avg_size = total_size_bytes / sampled_entries;
            total_size_bytes = avg_size * entry_count;
        }
        
        Ok(CacheStats {
            entry_count,
            total_size_bytes,
        })
    }

    /// Clear the cache
    pub fn clear(&self) -> Result<(), Box<dyn std::error::Error>> {
        self.db.clear()?;
        info!("Cleared summarizer cache");
        Ok(())
    }

    /// Get hit/miss counts and ratio
    pub fn get_hit_miss_stats(&self) -> (u64, u64, f64) {
        let hits = self.hit_count.load(std::sync::atomic::Ordering::Relaxed);
        let misses = self.miss_count.load(std::sync::atomic::Ordering::Relaxed);
        let ratio = if hits + misses > 0 {
            hits as f64 / (hits + misses) as f64
        } else {
            0.0
        };
        (hits, misses, ratio)
    }

    /// Reset hit/miss counters
    pub fn reset_counters(&self) {
        self.hit_count.store(0, std::sync::atomic::Ordering::Relaxed);
        self.miss_count.store(0, std::sync::atomic::Ordering::Relaxed);
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use kernel::{GuardrailsInfo, SummarizationMetrics};
    use tempfile::TempDir;

    #[test]
    fn test_cache_key_serialization() -> Result<(), Box<dyn std::error::Error>> {
        let key = CacheKey::new(
            "doc-123".to_string(),
            vec!["seg-1".to_string(), "seg-2".to_string()],
            "abstract_with_bullets".to_string(),
            true,
            6000,
            1000,
            200,
            "gpt-3.5-turbo".to_string(),
            "https://api.openai.com/v1".to_string(),
            "v1".to_string(),
        );
        let string_key = key.to_string_key()?;
        assert!(string_key.starts_with("sum:doc-123:"));
        
        let doc_id = CacheKey::doc_id_from_string_key(&string_key).unwrap();
        assert_eq!(doc_id, "doc-123");
        
        Ok(())
    }

    #[test]
    fn test_cache_operations() -> Result<(), Box<dyn std::error::Error>> {
        let temp_dir = TempDir::new()?;
        let cache_path = temp_dir.path().join("test_cache");
        
        let config = CacheConfig {
            enabled: true,
            path: cache_path.to_string_lossy().to_string(),
            ttl_days: 0, // 0 means no TTL
        };
        
        let cache = SummarizerCache::new(config)?;
        
        let cache_key = CacheKey::new(
            "test-doc".to_string(),
            vec!["seg-1".to_string(), "seg-2".to_string()],
            "abstract_with_bullets".to_string(),
            true,
            6000,
            1000,
            200,
            "gpt-3.5-turbo".to_string(),
            "https://api.openai.com/v1".to_string(),
            "v1".to_string(),
        );
        
        let response = SummarizeResponse {
            summary_text: "This is a test summary".to_string(),
            bullets: Some(vec!["Bullet 1".to_string(), "Bullet 2".to_string()]),
            citations: None,
            guardrails: Some(GuardrailsInfo {
                filtered: false,
                reason: None,
            }),
            metrics: SummarizationMetrics {
                processing_time_ms: 100,
                input_tokens: 1000,
                output_tokens: 50,
                cache_hit: false,
                mode: "test".to_string(),
                prompt_version: "v1".to_string(),
            },
        };
        
        // Test cache miss
        let result = cache.get_summary(&cache_key)?;
        assert!(result.is_none());
        
        // Test cache put
        cache.put_summary(&cache_key, &response)?;
        
        // Test cache hit
        let result = cache.get_summary(&cache_key)?;
        assert!(result.is_some());
        let cached_response = result.unwrap();
        assert_eq!(cached_response.summary_text, "This is a test summary");
        assert_eq!(cached_response.metrics.cache_hit, false); // This is set by the caller, not the cache
        
        // Test cache stats
        let stats = cache.get_stats()?;
        assert_eq!(stats.entry_count, 1);
        
        // Test cache clear
        cache.clear()?;
        let stats = cache.get_stats()?;
        assert_eq!(stats.entry_count, 0);
        
        Ok(())
    }

    #[test]
    fn test_disabled_cache() -> Result<(), Box<dyn std::error::Error>> {
        let config = CacheConfig {
            enabled: false,
            path: "./.cache/summarizer".to_string(),
            ttl_days: 0,
        };
        
        let cache = SummarizerCache::new(config)?;
        let cache_key = CacheKey::new(
            "test-doc".to_string(),
            vec!["seg-1".to_string()],
            "abstract_with_bullets".to_string(),
            false,
            6000,
            1000,
            200,
            "gpt-3.5-turbo".to_string(),
            "https://api.openai.com/v1".to_string(),
            "v1".to_string(),
        );
        
        let response = SummarizeResponse {
            summary_text: "This is a test summary".to_string(),
            bullets: None,
            citations: None,
            guardrails: None,
            metrics: SummarizationMetrics {
                processing_time_ms: 100,
                input_tokens: 1000,
                output_tokens: 50,
                cache_hit: false,
                mode: "test".to_string(),
                prompt_version: "v1".to_string(),
            },
        };
        
        // Operations should succeed but not actually cache
        cache.put_summary(&cache_key, &response)?;
        let result = cache.get_summary(&cache_key)?;
        assert!(result.is_none());
        
        Ok(())
    }
}