    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ModelResolutionError::DownloadsDisabled => {
                write!(
                    f,
                    "Network downloads are disabled and model files are missing"
                )
            }
...
pub async fn resolve_model(config: &AnalyzerConfig) -> Result<ResolvedModel, ModelResolutionError> {
    match &config.model {
        ModelConfig::HuggingFace(hf_config) => {
            resolve_huggingface_model(hf_config, config.allow_downloads).await
        }
        ModelConfig::Local(local_config) => resolve_local_model(local_config),
    }
}
...
async fn resolve_huggingface_model(
    ...
    let model_cache_dir = cache_dir.join("hub").join(format!(
        "models--{}--{}",
        hf_config.repo_id.replace('/', "--"),
        hf_config.revision
    ));
    ...
    if !missing_files.is_empty() {
        ...
        download_hf_files(hf_config, &model_cache_dir, &missing_files).await?;
...
fn resolve_local_model(
    ...
    info!(
        "Resolving model from local directory: {}",
    ...
    if !model_dir.exists() {
        return Err(ModelResolutionError::LocalModelAccess(format!(
    ...
    for file in &expected_files {
        ...
        if file_path.exists() && file_path.metadata()?.len() > 0 {
            ...
            return Err(ModelResolutionError::FileNotFound(format!(
                "Missing file: {}",
                file
            )));
        }
...
async fn download_hf_files(
    ...
    for file in files {
        ...
        if !response.status().is_success() {
            return Err(ModelResolutionError::DownloadFailed(format!(
                "HTTP {}: {}",
                response.status(),
                url
            )));
...
mod tests {
    ...
    #[tokio::test]
    async fn test_resolve_hf_model_no_downloads() {
        ...
        let result = resolve_huggingface_model(&hf_config, false).await;
        assert!(result.is_err());
